...
**REG1_MS1.pdf (règlement écrit Tome 1) re-ingestion**: every retrieved hit had `section=None` — traced to `data/index/chunks.json` predating the `section`/`page`/`norm_level`/`city` Chunk fields entirely (stale schema, not a chunker bug). Docling-extracted REG1_MS1.pdf (250p, 775 section_headers) and rebuilt its 655 chunks with `section` = real article code, replacing 666 stale ones; every other file's chunks byte-for-byte untouched (`scratch/ingest_reg1_docling.py`). Caught and fixed an NFD/NFC path-normalization bug before it could duplicate chunks instead of replacing them. Re-run: mean retrieval_score 0.0% → 25.0% (12 golden cases); movement confirmed attributable to sections entering the index, not scoring/ranking changes — Annexe/graphic-only cases (Tome 2, DG5 plans) correctly stayed at 0%, not ingested.

**corpus_mapping.yaml drift**: `classify_path()` returned `family="other"` for all 410 on-disk PDFs — rules keyed on `"PLU/75 Paris/PLU Bioclimatique/..."`, real tree is `"PLU bioclimatique/..."` (confirmed: this exact wrong path was even *reinforced* by the 2026-09-10 entry above, which "corrected" a script to match the already-stale yaml instead of the real disk). Fixed at the yaml layer (not `classify_path` — its NFC-normalization was already correct, verified directly with a synthetic NFD path). `other` count now 0/410 on disk, 0/411 in `corpus_metadata.json`. Added `check_corpus_mapping_coverage` (`check.py`, runs check "0", independent of `chunks.json` so it can't go stale-and-silent the way this bug did) + `checks/known_unmapped_files.json` allowlist + 3 regression tests including one against the real corpus.

**Open threads**: `checks/known_zero_chunk_files.json` / `known_encoding_failures.json` / `known_manifest_desync.json` all still store entries in the old, now-dead `PLU/75 Paris/...` path form — a separate, pre-existing allowlist-matching bug, found in passing, not fixed. Retrieval-ranking misses remain on 6/12 golden cases even with real sections (CH-01/02/03, UC-01/04/05) — a ranking/retrieval-quality question, not a metadata one; not chased further this session. Tome 2 (Annexe I/V/X, REG2A*) not yet ingested section-aware.

---

## 2026-09-11b — scratch/ fix: latent grouping bug from HOTEL_DIEU's stray-digit lines, found a genuine 2-case conflict before shipping (0 commits, scratch/ only)

**Done.** Fixed the residual flagged last entry: HOTEL_DIEU's "2.3.5" entry had a stray bare "2" line (a PDF rendering artifact) silently absorbed into its already-closed group, corrupting title→"...Plan C 30" and page→2. First attempt (drop any bare-digit-only line once closed) broke RP_INDIC.pdf's "3.1" entry, whose own raw shape is "État des lieux...BD COM" / "2023" / "27" — TWO consecutive bare-digit lines where "2023" is genuine title content (a database year) and "27" is the real page number; treating both as noise dropped the real page number too. Second attempt (require real content on the SAME line to count as a "close" at all) fixed RP_INDIC but broke RP_EVAL.pdf's "Bilan des effets...PLU" / "342" / "Mesures d'accompagnement..." (unnumbered, uppercase-started) — needs "342" alone to close immediately so the next entry's `starts_uppercase` check fires; requiring real content on "342" left it open and fused the next entry in. Landed on a two-tier fix: `current_closed` (any trailing digit, exactly as it always worked) still drives whether the next line can start a new entry; a NEW `current_closed_strong` flag (true only when the closing line itself had real content, not just a bare digit) separately gates the noise-drop — so RP_EVAL's "342" still closes-and-lets-next-start (weak close, no drop applies), RP_INDIC's "27" still gets appended (weak close from "2023", so absorption continues as it always did), and only HOTEL_DIEU's "2" (arriving after a genuinely complete, dot-leader'd row) gets dropped.

**Result**: HOTEL_DIEU's 2.3.5 now `titre='Evolution du Plan C'`, `page_debut=30` — force-verified directly against the real page text (not the random 10-sample, which doesn't include this entry) at offset 0, exact match. Full non-regression: all 13 other docs (REG1, REG2A1, REG2A10×2, RP_CHOIX, RP_DE, RP_DIAGNOSTIC, RP_EIE, RP_EVAL, RP_INDIC, RP_RNT, Rapport_presentation_MS1, RP_PREAMBULE) byte-identical — entries, anchor, AND warnings unchanged. PREAMBULE still correctly empty.

**Open threads**: none blocking. This is the third fix attempt landed in one turn — the two intermediate broken versions never left this session (caught by non-regression before being reported), but worth remembering for next time: a "drop when X" rule on shared grouping code needs to be checked against every doc that has ANY bare-digit-adjacent line, not just the one that motivated it.

---

## 2026-09-11 — scratch/ fix: marker-free sommaire detection (RP_20251017_MC1_HOTEL_DIEU) + sequence-gap warning (0 commits, scratch/ only)

**Done, "2nd task" deferred from the last session.** Fix 1: added `discover_toc_shape_start` — a density-based fallback (>=50% of a page's lines match "text...dot-leader...page-number") tried ONLY when the existing marker search (`discover_toc_start`) returns `None`, so any document with a real marker never reaches the new code path at all (by construction, not just by testing). Confirmed on the two "aucun sommaire" docs before trusting the threshold: HOTEL_DIEU's real TOC page is 94% dot-leader-shaped (30/32 lines) vs 0% on every other of its first 15 pages; RP_PREAMBULE (genuine absence) is 0% on all 9 of its pages — wide margin either side of the 50% floor. Fix 2: `detect_sequence_gaps`, symmetric to the existing `detect_duplicate_codes` (same parent-walk shape), flags a missing bare-integer sibling (e.g. root-level 1, 2, 4 — no 3) without renumbering; deliberately narrow to bare integers only, since dotted codes have no well-defined "next expected" value.

**Result**: RP_20251017_MC1_HOTEL_DIEU now detected (page 2 printed / index 1), 30 entries, levels 1-3, **anchor 100%**. Both new-warning types confirmed firing together: duplicate code `'2.3.1'` ×3 (p.26, p.36, p.37 — a genuine source restart, "Annexe IV"/"Annexe V" wrongly both labeled 2.3.1 instead of continuing 2.3.10/2.3.11) and sequence gap (section 3 absent, 2→4). Non-regression: all 13 other docs (REG1, REG2A1, REG2A10×2, RP_CHOIX, RP_DE, RP_DIAGNOSTIC, RP_EIE, RP_EVAL, RP_INDIC, RP_RNT, Rapport_presentation_MS1, RP_PREAMBULE) byte-identical — entries, anchor, AND warnings all unchanged; RP_PREAMBULE still correctly empty (shape detector did not hallucinate a sommaire there).

**Residual found, NOT fixed (out of today's scope, flagged not silently patched)**: entry `numero='2.3.5'` shows `page_debut=2` with a corrupted title (`"Evolution du Plan C 30"`) instead of the correct page 30. Root cause traced: a stray lone `"2"` line (a genuine PDF rendering artifact, confirmed via raw-line dump — not caused by today's fixes) sits immediately after "2.3.5. Evolution du Plan C ... 30" in the raw line stream; since it has no dot-leader/uppercase-start/numbering-prefix, `group_raw_lines`'s closed-branch logic appends it to the already-closed 2.3.5 group, and the trailing-digit extraction then grabs "2" as the page instead of "30". This is a pre-existing latent bug in the shared grouping logic (same family as the RP_DIAGNOSTIC callout-fusion fix, different trigger shape: a bare stray digit, not a statistic-led title) — never previously exposed because HOTEL_DIEU was never successfully extracted before this session. Anchor still reports 100% because this entry wasn't in the 10-sample.

**Open threads**: the "2.3.5" page-corruption bug needs its own scoped fix (a bare stray-digit line after a closed group is currently swallowed rather than dropped/flagged) — not attempted here, deliberately, to keep this session's diff isolated to the two requested fixes.

---

## 2026-09-10b — scratch/ fix: zone-code full-capture (RP_CHOIX "UGSU" dup) + RP_DIAGNOSTIC callout fusion, then full regeneration (0 commits, scratch/ only)

**Done, 3 isolated steps per instruction, each STOP-AND-SHOW'd before the next.** Step 1: added `RE_ZONE_SUBCODE` to `parse_prefix` — source lines like `"UGSU. 2.1 Dispositions Générales... 248"` put a SPACE between the zone prefix's dot and its sub-number (inconsistently — some entries even add a trailing dot: `"UGSU. 2.3."`), so `RE_CODE_DOTGROUPS`'s concatenated-only pattern fell through to `RE_LETTERS_DOT`, which grabbed just `"UGSU"` and left the real sub-number in the title (RP_CHOIX's 5-way `numero='UGSU'` duplicate). Verified on REG1 (0 zone codes there, byte-identical apart from a pre-existing/unrelated 4-entry level mismatch already traced to the stale saved tree, confirmed via `git stash` isolation) before touching RP_CHOIX. Step 2: audited RP_DIAGNOSTIC's "sidebar callout" hypothesis and found it WRONG — the "18,1% de logements inoccupés..." line sits in the exact same block/column as its neighbors, own dot-leader, own page number (82); it's a genuine standalone entry swallowed into the PRECEDING closed entry because the group-boundary fallback (`starts_uppercase`) only recognizes a leading LETTER, not a leading digit/percent sign. Fixed with a shape-based check (`looks_like_standalone_toc_entry`: dot-leader + own trailing page number, gated to the already-closed branch only, so it can't re-break a still-open wrap's closing line like RP_RNT's). First regex attempt (`\.{2,}`) missed because this PDF renders SPACED dots (". . . ."); fixed to `\.(?:\s?\.){2,}`. Isolated via the same revert/re-run technique — confirmed byte-identical on the other 8 target docs with and without the fix.

**Step 3 — full regeneration vs git-HEAD baseline (7b3d132), all 14 docs, every change classified.** Result: **zero unexpected changes anywhere.** REG1 (4) and REG2A1 (1): level-only, pre-existing/unrelated. RP_CHOIX: 5 zone-code extractions + 30 cascading level-only. RP_DIAGNOSTIC: 1 entry split (90%→100%). RP_EVAL: 43 "Tableau N." numero-upgrades (this session's predecessor fix, now correctly attributed). RP_INDIC: 12 level-only + 7 combined reorder+numero-upgrade. RP_RNT: 15 level-only + 18 pure reorder (exact multiset match, content provably unchanged). REG2A10×2/RP_DE/RP_EIE/RP_PREAMBULE/RP_20251017_MC1_HOTEL_DIEU/Rapport_presentation_MS1: byte-identical. All anchors now 100% except the 2 genuine "aucun sommaire" files (RP_PREAMBULE, HOTEL_DIEU — untouched, separate marker-detection scope). First diff pass used raw positional-index comparison and wrongly flagged ~100 changes as "unexpected" (an artifact of comparing lists by index instead of aligning them) — rebuilt with `difflib.SequenceMatcher` + explicit pattern recognizers before trusting any "unexpected" count; caught and fixed before reporting, not after.

**Open threads**: none blocking — all 14 `*_sommaire_tree.json` regenerated and consistent, ready for injection review. `scratch/step3_regenerate.py` kept as the reusable regeneration+diff-classification tool (not disposable).

---

## 2026-09-10 — scratch/ fix: RP_RNT/RP_EVAL/RP_INDIC 90%→100% anchor via `_decide_column_split` robustness + duplicate-code detection (0 commits, scratch/ only)

**Done**: audited the "multi-line title not rejoined" hypothesis for the 4 files at 90% anchor (RP_DIAGNOSTIC/EVAL/INDIC/RNT) before fixing — found the actual shared root cause was NOT continuation-joining (already correct) but `_decide_column_split` picking only the single largest x0-gap and using raw block-COUNT for balance. RP_RNT: a "SOMMAIRE" title block sits between its 2 real columns, fragmenting the true gap into two smaller ones that both fail the checks → single-column fallback → scrambled reading order → impossible `niveau=0` entries. RP_INDIC: its real 2nd column ("Tableau N." list) is just 2 large blocks vs many small ones on the left, failing the block-count floor. Fixed both generically in `extract_sommaire.py`: try every x0 gap largest-first (not just the biggest), and balance on total LINE count, not block count. Also added a generic `Tableau N.` numbering profile (was silently swallowing into an unrelated heading), a `niveau<1` loud-warning guard (never silently ships one), and `detect_duplicate_codes` (flags, never renumbers, a same-code sibling collision — confirmed genuine source-doc error on RP_RNT: "3.1" repeats at p.21 and p.50, should be 3.6). Also fixed `normalize()` to de-hyphenate PDF line-wraps ("cli-\nmatique"), which the reorder incidentally exposed as a separate anchor-check gap. Corrected `run_consolidated_extraction.py`'s hardcoded `Ressources/PLU bioclimatique/` path (stale — real layout is `Ressources/PLU/75 Paris/PLU Bioclimatique/`, per `corpus_mapping.yaml`) and `chemin_relatif` to use `.as_posix()` (was leaking Windows backslashes).

**Result**: RP_RNT 90%→**100%** (24 entries, 0 niveau<1, 1 duplicate-code warning), RP_EVAL 90%→**100%** (202 entries), RP_INDIC 90%→**100%** (19 entries). RP_DIAGNOSTIC stayed at 90% — confirmed a genuinely different, unrelated cause (a sidebar stat callout fused into one TOC title by y-order proximity, not column/join related) — correctly left untouched, not silently fixed nor worsened. Non-regression: REG1/REG2A1/REG2A10×2/RP_CHOIX/Rapport_presentation_MS1 all byte-identical on (numero, titre, page_debut) and anchor (100% throughout); REG1/REG2A1's pre-existing niveau-histogram mismatch against their saved trees predates this session (confirmed identical before/after my edits, inherited from the prior consolidation commit, not something this session introduced). One expected new signal: RP_CHOIX gained 1 duplicate-code warning ("UGSU" repeats 5× under section 1.5.2) — flagged as likely a DIFFERENT, pre-existing issue (a zone-prefix code being captured instead of the real per-item sub-number, unlike RNT's genuine source-doc duplication) — not fixed, out of this task's scope.

**Open threads**:
- RP_CHOIX's "UGSU" duplicate-code warning is probably a numbering-capture granularity issue (RE_LETTERS_DOT grabbing only the zone prefix, dropping the real "2.1"/"2.2" sub-number into the title), not a source-document error like RP_RNT's — worth a real look before this feeds anything downstream, but out of scope here.
- RP_DIAGNOSTIC's 90% (sidebar-callout title contamination) and RP_EVAL's residual title/anchor edge cases beyond the ones fixed here are undiagnosed beyond what's in this entry — not chased further (different root cause, "one fix at a time").
...
## 2026-07-29 — Full-corpus PDF inventory (412 rows) linked to family ontology (read-only, 0 commits)

**Context**: Notion has two documentary-ontology databases — "Ontologie documentaire — Corpus PLU" (12 rows, family-level, `Fichiers (nb)` sums to 409) and "Ontologie documentaire — Niveau fichier" (51 rows, the already-characterized textual files). Task: one row per PDF on disk, mapped to the 12-family relation key, reusing the 51-file characterization verbatim rather than re-detecting.

**Sanity check, mismatch found and explained**: disk has **412** PDFs, not 409. The +3 are `REG1.pdf`, `REG2A1.pdf`, `REG2A10_1DE2.pdf` — pre-MS1 duplicates sitting alongside their `_MS1` twins, already documented in `corpus_mapping.yaml` as `validity: superseded` (byte-identical, page-by-page diff verified). Confirmed via page-count match (250/250, 224/224, 700/700). Included all 412 in the inventory (task requires 1 row per file that exists) and reused each superseded file's `_MS1` twin structure_native/pages rather than re-detecting. Also noted (not an error): `LEG2000.pdf` is the same legend file physically duplicated across 3 Atlas subfolders — 412 rows, 410 distinct filenames, disambiguated by `chemin_relatif`.

**Method for the ~361 graphic/scan files** (326 Atlas + 12 PPRI + 14 Plans autres périmètres + 6 Plans SUP): page counts via `pypdf` for all (cheap, no OCR). Sampled first/mid/last file per subfolder (17 subfolders) to confirm near-zero real prose — confirmed: extracted "text" is scattered map-label/parcel-number/street-name fragments, not sentences, in every sampled file. **Caught 3 real exceptions this way** (index/consultation documents living inside Atlas folders, not zoning plates): `Consultation_des_planches_d_ilots_du_secteur_de_Montmartre.pdf`, `Consultation_des_plans_des_secteurs_de_maisons_et_villas.pdf`, `Consultation_des_planches_au_1_sur_2000.pdf` (the third wasn't in the original 3-per-folder sample — caught via a follow-up full first-page scan of all 358 graphic-family files using a word-count heuristic, then manually verified real prose before trusting it — the heuristic itself had false positives, e.g. `Secteur maisons et villas/P51.pdf` scored high on word count but manual read confirmed it's legend/street-label text, correctly left as `image_plan`). These 3 got `structure_native=liste`, `indexable_texte=oui` instead of the default `image_plan`/`non`.

**One reuse-verbatim exception flagged, not silently resolved**: `ANN2A_2025_12_19.pdf` is in the 51-file Notion table (`structure_native=narratif`, reused verbatim per the task's instruction) but `corpus_mapping.yaml` documents it as `validity: excluded` — its extracted "text" is only a repeated title banner, confirmed by the indexer's own page-by-page audit; Notion's own row already carries `À reclasser=oui`. Kept `structure_native=narratif` (verbatim, per instructions) but set `indexable_texte=non` (this task's own column-7 criterion, independently derived) — flagging the inconsistency rather than picking one value and hiding the conflict.

**Output**: `eval/ontology/corpus_pdf_inventory_full.csv`, 412 rows, sorted by famille then pdf. 0 `À CLASSER` rows — every file resolved cleanly to one of the 12 families by folder-prefix rule (deterministic, no guessing needed). 56 `indexable_texte=oui` / 356 `non`.

**Open threads**: `titre`, `niveau_integration`, `note` columns left empty for Charline/Wenwen per the task; the `ANN2A` narratif/non-indexable inconsistency is worth a real reclassification decision, not just a flagged row.

---

## 2026-07-29 — Business-review findings audit vs. current pipeline (read-only, 0 commits)

**Context**: Charline reviewed a Figma demo (older pipeline state, pre-T2..T7) and raised 5 findings (F1-F5). Audited each live (backend=ollama, expansion=gemma3:4b, synthesis=ministral-3:8b, top_k=10, scoped_retrieval on) before greenlighting a "restitution" work block.

**F1 (cite-then-declare-incompetent) — ALREADY FIXED.** Live re-run of the exact "volets" question: answer cites `[2] OAP_HERITAGE.pdf` cleanly, structures as case (a)+(b) (rule + open points to verify), zero "le contexte ne précise pas" for the cited point. Fixed by `960f906`/`6a4377a` (2026-07-16 `SYSTEM_PROMPT` rewrite, `llm.py`) — the exact anti-contradiction clause the task asked about ("tu ne peux pas déclarer... une absence d'information" after citing). Figma demo predates this fix.

**F2 (citations too short) — ALREADY FIXED.** `api.py` `Citation.excerpt` sends full `hit.content` (confirmed live: 1200-char chunk -> 1200-char excerpt, e.g. UG.3.1.2 p.65), no `textwrap.shorten` cap since `6a4377a`. One residual, separate gap (not what F2 describes, flagging anyway): long articles spanning multiple chunks (e.g. UG.3.1.1 = 7 chunks) still surface as N independent citations, not one stitched article — open [SPLIT] thread from the same 07-16 audit, unaddressed.

**F3 (no section title/chapter context) — HALF FIXED, HALF OPEN, cheap.** Article **code** (`section`, e.g. "UG.3.1.2") is already in `Citation` and confirmed returned. Human-readable document **title** is NOT surfaced: `referentiel.yaml`'s `Piece.official_name` (e.g. "Règlement — Pièces écrites — Tome 1") exists and `api.py` already resolves `piece_id_by_abs_path` for `document_url`, but `official_name` is never joined into the `Citation` response. Confirmed backend-actionable, cheap (a lookup + one new field), not touched.

**F4 (false "je ne sais pas" instead of general UG rule) — STILL TRUE, retrieval-side, isolated.** Live re-run: query expansion correctly infers `UG.3.1.2 UG.3.1.3 UG.3.1.4` (gemma3:4b working as intended). But final ranked hits bury the real prospect articles — direct probe shows `N.7.2` (wrong zone family) occupying most of the 6 `reglement_ecrit` slots, only one weak `UG.3.1.2` hit (rank 10, score 0.035), `UG.3.1.3`/`UG.3.1.4` never surfacing. Synthesis correctly reports case (c) given what it actually received — not a synthesis bug. Same root cause as T7's already-open "ordinary UG-sub-article ranking granularity" thread (UC-04/CH-01/CH-02) — F4 is a new symptom of that same known, unaddressed gap.

**F5 (OAP perimeter not filtered) — STILL TRUE, confirmed unfixed.** `git log -S "oap" -- retriever.py` shows zero commits since the 2026-07-16 investigation; neither option (A) hard-exclude nor (B) boost-regex fix was ever applied. Direct probe (`"...2e arrondissement ?"`, `--family oap`) surfaces `OAP_PORTES_EST.pdf` (a 12e/20e-only sectoral OAP) in the top 10 — reproduces the exact complaint. `oap` slot is still 1 (07-16 "don't raise slot count" verdict stands; that's a separate, already-closed question from the filtering question itself).

**Decision**: none — audit only. Backend-actionable next steps, in order of cost: F3's title-join (cheap, no re-index) -> F5's perimeter hard-exclude (no re-index, static filename->arrondissement dict per the 07-16 design) -> F4's ranking-granularity fix (harder, same class as T7's still-open thread) -> F2's [SPLIT] stitching (harder, touches `marker_index` design).

**Open threads**: F3/F5 fixes are cheap and ready to scope as a single "restitution" task; F4 needs its own ranking investigation (do NOT bundle with F3/F5 — different root cause, per "one fix at a time").
